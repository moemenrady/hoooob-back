    ...
    public function index(array $criteria = [], array $relations = [], array $whereHasRelations = [], array $orderBy = [], int $limit = null, int $offset = null, array $withCountQuery = [], array $appends = [], array $groupBy = []): Collection|LengthAwarePaginator
    {
        ...
        if (array_key_exists('search', $criteria) && $criteria['search'] != '') {
            ...
            $searchData['relations'] = [
                'NewDriver' => ['full_name', 'first_name', 'last_name', 'email', 'phone'],
            ];
 ...
 public function create(array $data): ?Model
{
    ...
    $carImageFields = [
        'front_car_licence_image',
        'back_car_licence_image',
        'front_driver_licence_image',
        'back_driver_licence_image'
    ];

    foreach ($carImageFields as $field) {
        if (!empty($data[$field]) && $data[$field] instanceof \Illuminate\Http\UploadedFile) {
            $file = $data[$field];
            $extension = $file->getClientOriginalExtension();
            $documents['car_data'][$field] = fileUploader('vehicle/document/', $extension, $file);
     ...
     if (!empty($data['other_documents']) && $data['other_documents'] instanceof \Illuminate\Http\UploadedFile) {
        $file = $data['other_documents'];
        $extension = $file->getClientOriginalExtension();
        $documents['other_document'] = fileUploader('vehicle/document/', $extension, $file);
        unset($data['other_documents']);
    }
