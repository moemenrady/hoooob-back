...
class TripRequestRepository implements TripRequestInterfaces
{
    ...
    {
        $rideCount = auth()->user()->driverDetails->ride_count ?? 0;
        $parcelCount = auth()->user()->driverDetails->parcel_count ?? 0;
        $parcelFollowStatus = $attributes['parcel_follow_status'] ?? false;
        $maxParcelRequestAcceptLimitCount = $attributes['max_parcel_request_accept_limit'] ?? 0;
        $query = $this->trip
            ->when($attributes['relations'] ?? null, fn($query) => $query->with($attributes['relations']))
            ->with([
            ...
            ->where(function ($query) use ($rideCount, $parcelCount, $parcelFollowStatus, $maxParcelRequestAcceptLimitCount) {
                if ($rideCount < 1) {
                    $query->where('type', RIDE_REQUEST);
                }

                // 2. Parcel request logic based on parcel follow status and parcel count
                $query->orWhere(function ($query) use ($parcelFollowStatus, $parcelCount, $maxParcelRequestAcceptLimitCount) {
                    if ($parcelFollowStatus) {
                        // Only include parcels if parcel_count < 2
                        if ($parcelCount < $maxParcelRequestAcceptLimitCount) {
