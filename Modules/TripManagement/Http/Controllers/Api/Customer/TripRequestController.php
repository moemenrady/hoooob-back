 ...
 public function createRideRequest(Request $request): JsonResponse
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'required_seats'=>'sometimes|numeric|min:1|max:4',
...
//        dd($pickup_coordinates);
            ...
            if ($request->is_carpool == true) {
                try {
                    $save_trip = DB::transaction(function () use ($request, $route) {
                        $lockedRoute = CarpoolRoute::where('id', $route->id)->lockForUpdate()->first();
                        if (!$lockedRoute || (int)($lockedRoute->seats_available ?? 0) < 1) {
                            throw new \Exception('No seats available');
                        }
                        $lockedRoute->seats_available = (int)$lockedRoute->seats_available - 1;
                        $lockedRoute->save();
    ...
    /**
     * Snap a coordinate [lng, lat] to the nearest point on a route polyline [{lat, lng}, ...]
     */
