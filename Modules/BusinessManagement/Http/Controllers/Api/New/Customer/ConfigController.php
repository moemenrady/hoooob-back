...
class ConfigController extends Controller
{
    ...
    public function configuration()
    {
        ...
        $configs = [
            ...
            'maximum_parcel_weight_status' => (bool) $info->firstWhere('key_name', 'max_parcel_weight_status')?->value == 1 ? true : false,
...
//        $response = Http::get(MAP_API_BASE_URI . '/place/autocomplete/json?input=' . $request['search_text'] . '&key=' . $mapKey . '&components=country:IN');
  ...
  public function getRoutes(Request $request)
    ...
    public function geocodeApi(Request $request): JsonResponse
    {
        ...
        $mapKey = businessConfig(GOOGLE_MAP_API)?->value['map_api_key_server'] ?? null;
//        $response = Http::get(MAP_API_BASE_URI . '/geocode/json?latlng=' . $request->lat . ',' . $request->lng . '&key=' . $mapKey . '&components=country:IN');
